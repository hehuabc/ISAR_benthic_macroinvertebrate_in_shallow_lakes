...
data2 <-  mutate(data,
                ...
                s_n = log10(Sn),
                s_pie = log10(Spie),
                s_error = 1/(s_area_UCI - s_area_LCI+1), # reversal CI for weights
                sn_error = 1/(Sn_UCI - Sn_LCI+1), # reversal CI for weights
                spie_error = 1/(Spie_UCI - Spie_LCI+1), # reversal CI for weights
                density = log10(abundance)) %>% setDT()

data2$climate <- factor(data2$climate,labels = c("Cool","Warm"))

# fit_sarea
fit_s <- glmmTMB(s_a ~  Area  + (Area|source) + (1|lat) + (1|long),
                  weights = s_error,
                  data2)
summary(fit_s)
...
# fit_sn
fit_sn <- glmmTMB(s_n ~  Area  + (Area|source) + (1|lat) + (1|long),
                  weights = sn_error,
                  data2)
summary(fit_sn)
...
# fit_spie
fit_spie <- glmmTMB(s_pie ~ Area + (Area|source) + (1|lat) + (1|long),
                  weights = spie_error,
                  data2)
...
# s_int
fit_s_int <- glmmTMB(s_a ~ Area * climate + (Area|source)+(1|lat) +(1|long),
                      weights = s_error,
                      data2)
summary(fit_s_int)
...
# sn_int
fit_sn_int <- glmmTMB(s_n ~ Area * climate + (Area|source)+(1|lat) +(1|long),
                      weights = sn_error,
                    data2)
summary(fit_sn_int)
...
# Spie-int
fit_spie_int <- glmmTMB(s_pie ~ Area * climate +(1|source)+(1|lat) +(1|long),
                     weights = spie_error, 
                     data2)
...
fit_tp_int <- glmmTMB(TP ~ Area*climate + (1|source) + (1|lat) + (1|long),
                  data2)
...
# tn
fit_tn_int <- glmmTMB(TN ~ Area * climate  + (1|source) + (1|lat) + (1|long), 
                  data2)
...
fit_n_int <- glmmTMB(density ~  Area * climate +
                       (1|TN) + (1|TP) + (1|lat) +(1|long),
               data2)
...
plot_s <-  ggplot(data= data2,aes(Area,s_a)) +
  geom_point(aes(shape = climate,color = source),size = 0.5,alpha = 0.8) + 
  ...
  geom_smooth(aes(color = source),show.legend = F,
              ...
              se = F,
              size = 0.3)  +
  geom_line(data = data_s, aes(x, predicted),size = 1,linetype = "dashed") +
  ...
  annotate("text",x = 3, y = 2, size = 3,
           label = expression(beta~' = 0.024'))+
...
plot_sn <-  ggplot(data= data2,aes(Area,s_n)) +
  ...
  geom_smooth(aes(color = source),show.legend = F,
              ...
              se = F,
              size = 0.3)  +
  geom_line(data = data_sn, aes(x, predicted),size = 1,linetype = "dashed") +
...
plot_spie <-  ggplot(data= data2,aes(Area,s_pie)) +
  ...
  geom_smooth(aes(color = source),
              ...
              se = F,
              size = 0.3)  +
  geom_line(data = data_spie, aes(x, predicted),size = 1,linetype = "dashed") +
  ...
  theme(legend.position = "none",
        legend.background = element_blank()) +
  annotate("text",x = 3, y = 1.5,size = 3, 
           label = expression(beta~' = 0.033'))
...
plot_s_int <-  ggplot(data = data_s_int, aes(x, predicted))+
  ...
  annotate("text",x = 3, y = 2,size = 3, 
           label = expression(beta[Warm]~'= 0.057'~~~beta[Cool]~'= - 0.014')) 
plot_s_int
...
plot_sn_int <-  ggplot(data = data_sn_int, aes(x, predicted))+
  ...
  scale_y_continuous(limits = c(0,2)) +
  annotate("text",x = 3, y = 2,size = 3, 
           label = expression(beta[Warm]~'= 0.154'~~~beta[Cool]~'= - 0.024'))
plot_sn_int
...
plot_spie_int <-  ggplot(data = data_spie_int, aes(x, predicted))+
  ...
  annotate("text",x = 3, y = 1.5,size = 3, 
           label = expression(beta[Warm]~'= 0.078'~~~beta[Cool]~'= -0.001'))
plot_spie_int
...
plot_n <-  ggplot(data= data2, 
  ...
  geom_point(aes(color = climate),
             size = 1,alpha = 0.4, shape = 16) +
  geom_line(data = data_n_int, aes(x, predicted,color = group),size = 0.8) +
  ...
  scale_color_manual(values = c( "mediumpurple","darkorange1")) +
  scale_fill_manual(values = c("mediumpurple","darkorange1")) +
  annotate("text",x = 3,y = 5.2,size = 3,
           label = expression('Interaction':~italic(P)== '0.025')) +
  annotate("text",x = 3, y = 4.9,size = 3, 
           label = expression('Area':~italic(P)< '0.001'~~~"Climate":~italic(P)<"0.001"))
plot_n
...
plot_area_tp <-  ggplot(data= data2, aes(Area,TP))+ 
  geom_point(aes(color = climate),
             size = 1,alpha = 0.4, shape = 16) +
  geom_line(data = data_tp_int, aes(x, predicted,color = group),size = 0.8,
            linetype= "dashed") +
  ...
  scale_color_manual(values = c("mediumpurple","darkorange1")) +
  scale_fill_manual(values = c("mediumpurple","darkorange1")) +
  annotate("text",x = 3,y = 0.8,size = 3,
           label = expression('Interaction':~italic(P)== '0.212')) +
  annotate("text",x = 3, y = 0.5,size = 3, 
           label = expression('Area':~italic(P)== '0.056'~"Climate":~italic(P)=="0.142"))
plot_area_tp


summary(fit_tn_int)
plot_area_tn <-  ggplot(data= data2, aes(Area,TN))+ 
  geom_point(aes(color = climate),size = 1,alpha = 0.4,shape = 16) +
  geom_line(data = data_tn_int, aes(x, predicted,color = group),size = 0.8,
            linetype= "dashed") +
  ...
  scale_color_manual(values = c("mediumpurple","darkorange1")) +
  scale_fill_manual(values = c("mediumpurple","darkorange1")) +
  annotate("text",x = 3,y = 1.2,size = 3,
           label = expression('Interaction':~italic(P)== '0.740')) +
  annotate("text",x = 3, y = 1,size = 3, 
           label = expression('Area':~italic(P)== '0.095'~~"Climate":~italic(P)=="0.975"))
plot_area_tn

# Figure 3 arrange
tiff("Figure3.tiff",res = 900,compression = "lzw",width = 7,height = 20,
     units = "cm")
